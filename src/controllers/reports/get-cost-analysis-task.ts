...
export const getCostAnalysisTask = async (req: Request, res: Response) => {
  ...
  try {
    let response: { status: string; code: number; data?: any; message?: string } = await execProc("ProcTaskRelAnaliseCustoTarefa", params);
